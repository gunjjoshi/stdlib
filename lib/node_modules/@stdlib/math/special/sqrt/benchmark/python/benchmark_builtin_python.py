...
"""Benchmark Python using only built-ins."""
...
def benchmark(name, setup, stmt, iterations):
    ...
    * `iterations`: number of iterations
    ...
    i = 0
    while i < REPEATS:
        print("# python::builtin::" + NAME + name)
        COUNT[0] += 1
...
def main():
    ...
    while p <= MAX:
        ...
        p += 1
        name = ":len="+str(n)
        setup = "from math import sqrt; import numpy as np; import numpy.random;"
        setup += "x = numpy.random.uniform(0.0, 10.0, ("+str(n)+")).astype('float64');"
        stmt = "y = np.zeros("+str(n)+");\n"
        stmt += "for i in range(y.size):\n"
        stmt += "  y[i] = sqrt(x[i]);"
