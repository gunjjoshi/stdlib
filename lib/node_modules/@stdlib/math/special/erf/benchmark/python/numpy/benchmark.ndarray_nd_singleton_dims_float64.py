...
def benchmark(name, setup, stmt, iterations):
    ...
    * `iterations`: number of iterations
    ...
    ``` python
    python> benchmark("::random", "from random random;", "y = random()", 1000000)
    ```
    """
...
def main():
    ...
    while p <= MAX:
        ...
        setup += "x = np.ndarray(shape=("+str(n)+", 1, 1), strides=(8, 8, 8), buffer=np.random.uniform(-1.0, 1.0, ("+str(n*2)+")), dtype='float64');"
        stmt = "y = erf(x)"
