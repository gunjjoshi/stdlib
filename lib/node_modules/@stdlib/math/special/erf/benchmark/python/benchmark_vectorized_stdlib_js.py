...
"""Benchmark stdlib JavaScript erf implementation via Node.js subprocess."""
...
def benchmark(name, setup, stmt, iterations):
    ...
    * `iterations`: number of iterations
    ...
    i = 0
    while i < REPEATS:
        print("# python::stdlib_js::" + NAME + name)
        COUNT[0] += 1
...
def create_node_script(n):
    """Create a Node.js script to compute erf for n random values."""
    script = '''
...
for (let i = 0; i < ''' + str(n) + '''; i++) {
    x.push(Math.random() * 2.0 - 1.0);
}

// Compute erf for all values
const y = x.map(erf);

// Return results (not used in benchmark, just for correctness)
...
def main():
    ...
    while p <= MAX:
        n = 10**p
        p += 1
        name = ":vectorized=true,method=stdlib_js_via_node,len="+str(n)
        
        # Create temporary Node.js script
        script_content = create_node_script(n)
        ...
        setup = "import subprocess, tempfile, os"
        stmt = "run_node_script()"
        
        # Add the function to the global namespace for timeit
        globals()['run_node_script'] = run_node_script
