...
tape( 'attached to the main export is an `assign` method', function test( t ) {
	t.strictEqual( typeof erf.assign, 'function', 'returns expected value' );
	t.end();
});
...
tape( 'the function computes the error function for each element in an ndarray for each element in an ndarray and assigns the results to a provided output ndarray', function test( t ) {
	var expected;
	var out;
	var x;
	var y;

	x = random( [ 5, 5 ], -5.0, 5.0, {
		'dtype': 'float64'
	});
	y = zeros( getShape( x ), {
		'dtype': 'float64'
